...
export async function getNewsContentsInfo(contentsId: string) {
    //   useEffect(()=>{
    ...
    const contentsRef = doc(db, "newscontents", contentsId)
...
export async function getNewsContentsCollectionInfo() {
    const contentsRef = collection(db, "newscontents")
...
export async function createOrupdateNewsContentsInfo(contentsId: string, data: {
    dialogue?: {
        speaker: string;
        line: string;
    }[]
...
export async function getRecommendedNewsContents(userId: string, n: number = -1) {//test用に軽量化
    // ① ユーザーデータ取得
    const userData = await getUserInfo(userId)
    if (!userData || !userData.preference || !userData.alreadyRead) {
        console.warn("User data is incomplete")
